...
<form id="evaluateForm" action="" method="post">
	<div class="pgxq_table">
	    ...
	    <div class="pgxq_text c6 f14">
			<dl>		
			<dd>课程评分：
			<span><input name="course_score"  value="${course_score}" type="text" class="f14 input_1 green" size="10" maxlength="3">
			</span></dd>
			<dd>课件评分：
			<span><input name="courseware_score" value="${courseware_score}" type="text" class="f14 input_1 green" size="10" maxlength="3">
			</span>
				...
				<div class="wj_text c6 f14">
					<dl>
						<c:forEach items="${q.options }" var="o" varStatus="ost">
							<dd>
								<label for="checkbox_b${q.id }${ost.count}">${q.optionCount }								
								<input name="checkbox_b${q.id }" ${q.type==2?"type='checkbox'":"type='radio'" } id="checkbox_b${q.id }${ost.count}"/><i>✓</i>
								<span>${o.optionKey }、${o.optionValue }</span>
								<span style="float:right">${o.optionCount}&nbsp;&nbsp;&nbsp;&nbsp;票</span>	
								</label>
							</dd>
						</c:forEach>
					</dl>
